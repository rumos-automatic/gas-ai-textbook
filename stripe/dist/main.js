 ...
 */
function recordSalesForDateRange(startTimestamp, endTimestamp) {
    // Charge一覧を取得
...
function createSalesRecord(charge) {
    ...
    // Checkout Sessionから顧客名と商品名を取得
    let customerName = '';
    let productName = '';
    let email = charge.billing_details?.email || '';
    if (charge.payment_intent) {
        const sessions = listCheckoutSessionsByPaymentIntent(charge.payment_intent);
        if (sessions.length > 0) {
            const sessionWithItems = getCheckoutSessionWithLineItems(sessions[0].id);
            if (sessionWithItems) {
                // 顧客名（決済フォームで入力された名前）
                if (sessionWithItems.customer_details?.name) {
                    customerName = sessionWithItems.customer_details.name;
                }
                // メール（Checkout Sessionから優先取得）
                if (sessionWithItems.customer_details?.email) {
    ...
    // フォールバック（Checkout Sessionから取得できなかった場合）
    if (!customerName) {
        // Customer APIから取得を試みる
        if (charge.customer) {
        ...
        // それでも取得できなければbilling_details（カード名義）を使用
        if (!customerName) {
 ...
 * Chargeのステータスを日本語に変換
 */
function translateChargeStatus(status) {
    const statusMap = {
        'succeeded': '成功',
        'pending': '保留中',
        'failed': '失敗'
 ...
 */
function testRecordSpecificDate(dateString) {
 ...
 */
function debugStripeConnection() {
...
function onOpen() {
    ...
    ui.createMenu('Stripe売上管理')
        .addItem('当日分を記録', 'recordTodayWithAlert')
        .addItem('前日分を記録', 'dailyRecordSalesWithAlert')
        .addSeparator()
 ...
 * 当日分の売上を記録（UIアラート付き）
 */
function recordTodayWithAlert() {
    const ui = SpreadsheetApp.getUi();
    try {
        log('INFO', '当日分の売上を記録します（メニューから実行）');
        const today = new Date();
        const dateRange = getDateRange(today);
        ...
        // Charge一覧を取得
        const charges = listCharges(dateRange.start, dateRange.end);
        if (charges.length === 0) {
            ui.alert('結果', `${dateStr} の決済はありませんでした。`, ui.ButtonSet.OK);
            return;
        }
        // 既存の決済IDを取得（重複チェック用）
        const existingChargeIds = getExistingChargeIds();
        ...
        const message = `【${dateStr} の処理結果】\n\n` +
            `取得件数: ${charges.length}件\n` +
            `新規追加: ${newRecords.length}件\n` +
            `スキップ（重複）: ${skippedCount}件`;
        ui.alert('完了', message, ui.ButtonSet.OK);
    }
    catch (e) {
        log('ERROR', '当日分の売上記録でエラーが発生しました', { error: String(e) });
        ui.alert('エラー', `エラーが発生しました: ${String(e)}`, ui.ButtonSet.OK);
    }
}
/**
 * 前日分の売上を記録（UIアラート付き）
 */
function dailyRecordSalesWithAlert() {
    const ui = SpreadsheetApp.getUi();
    try {
        log('INFO', '前日分の売上を記録します（メニューから実行）');
        const dateRange = getYesterdayDateRange();
        ...
        log('ERROR', '前日分の売上記録でエラーが発生しました', { error: String(e) });
        ui.alert('エラー', `エラーが発生しました: ${String(e)}`, ui.ButtonSet.OK);
    }
}
