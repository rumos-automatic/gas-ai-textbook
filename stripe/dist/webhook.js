 ...
 */
function processStripeWebhook(payload, signatureHeader) {
    // 署名検証（ヘッダーがある場合）
    if (signatureHeader) {
...
function handleCheckoutSessionCompleted(event) {
    ...
    // 対象商品かチェック
    const matchingItem = lineItems.find(item => item.price && eligiblePriceIds.includes(item.price.id));
    if (!matchingItem) {
 ...
 */
function getCheckoutSessionWithLineItemsAndPrices(sessionId) {
    if (!sessionId)
        return null;
    try {
        const response = stripeApiRequest(`/checkout/sessions/${sessionId}?expand[]=line_items.data.price`);
        return response;
    }
    catch (e) {
