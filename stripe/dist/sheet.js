 ...
 */
function isDuplicate(chargeId, existingIds) {
 ...
 */
function appendSalesRecords(records) {
    if (records.length === 0)
        return;
    const sheet = getSalesSheet();
    const lastRow = sheet.getLastRow();
 ...
 * メール送信ログシートの初期設定
 */
function setupEmailLogSheet(sheet) {
    // ヘッダー行を設定
 ...
 */
function isEmailAlreadySent(sessionId) {
 ...
 */
function markEmailSentForSession(sessionId, email, productName) {
    const sheet = getEmailLogSheet();
 ...
 */
function getRecentEmailLogs(limit = 10) {
    const sheet = getEmailLogSheet();
    const lastRow = sheet.getLastRow();
    if (lastRow <= 1) {
        return [];
