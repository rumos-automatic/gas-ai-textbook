 ...
 */
function listCheckoutSessionsByPaymentIntent(paymentIntentId) {
    if (!paymentIntentId)
        return [];
    try {
        const response = stripeApiRequest(`/checkout/sessions?payment_intent=${paymentIntentId}`);
 ...
 */
function getCheckoutSessionWithLineItems(sessionId) {
    if (!sessionId)
        return null;
    try {
        // customer_detailsは展開不要（デフォルトで含まれる）、line_itemsのみ展開
        return stripeApiRequest(`/checkout/sessions/${sessionId}?expand[]=line_items`);
    }
    catch (e) {
