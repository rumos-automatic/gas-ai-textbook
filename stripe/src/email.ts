 ...
 */
function sendDiscordInvitationEmail(
 ...
 * Discord招待リンクを取得
 */
function getDiscordInviteLink(): string {
 ...
 * プレーンテキスト版メール本文を取得
 */
function getEmailBody(customerName: string): string {
 ...
 * HTML版メール本文を取得
 */
function getEmailHtmlBody(customerName: string): string {
  const greeting = customerName ? `${customerName} 様` : 'お客様';
  const discordLink = getDiscordInviteLink();
 ...
 * GASエディタから実行してテスト
 */
function testSendDiscordEmail(): void {
