...
/**
 * Payment IntentからCheckout Sessionを検索
 * @param paymentIntentId Payment Intent ID
 */
function listCheckoutSessionsByPaymentIntent(paymentIntentId: string): StripeCheckoutSession[] {
  if (!paymentIntentId) return [];

  try {
    const response = stripeApiRequest(`/checkout/sessions?payment_intent=${paymentIntentId}`) as StripeListResponse<StripeCheckoutSession>;
    return response.data;
  } catch (e) {
    log('WARN', `Checkout Session検索失敗: ${paymentIntentId}`, { error: String(e) });
    return [];
  }
}

/**
 * Checkout Sessionを取得（line_items展開）
 * @param sessionId Checkout Session ID
 */
function getCheckoutSessionWithLineItems(sessionId: string): StripeCheckoutSession | null {
  if (!sessionId) return null;

  try {
    // customer_detailsは展開不要（デフォルトで含まれる）、line_itemsのみ展開
    return stripeApiRequest(`/checkout/sessions/${sessionId}?expand[]=line_items`) as StripeCheckoutSession;
  } catch (e) {
