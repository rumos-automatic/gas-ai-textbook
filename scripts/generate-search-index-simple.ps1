...
foreach ($dir in $targetDirs) {
    ...
    if (Test-Path $dirPath) {
        Write-Host "フォルダをスキャン中: $dir" -ForegroundColor Yellow

        $htmlFiles = Get-ChildItem -Path $dirPath -Filter "*.html" -File

        foreach ($file in $htmlFiles) {
            Write-Host "  処理中: $($file.Name)" -ForegroundColor Gray
            ...
            if ($sections.Count -gt 0) {
                ...
                # カテゴリ情報を取得
                $categoryIcon = $categoryIcons[$dir]
                if (-not $categoryIcon) { $categoryIcon = "icon-file" }

                $categoryColor = $categoryColors[$dir]
                if (-not $categoryColor) { $categoryColor = "gray" }
...
foreach ($specialFile in $specialFiles) {
    ...
    if (Test-Path $filePath) {
        Write-Host "処理中: $specialFile" -ForegroundColor Yellow
...
# JSONに変換して保存
$jsonContent = $indexData | ConvertTo-Json -Depth 10
...
Write-Host "ファイルサイズ: $fileSize KB" -ForegroundColor White
Write-Host ""
